...
(function (global) {
  ...
  const KB = [
    ...
    },
    {
      ...
      answer(ctx) {
        ...
        const head = '量表在「设置 → 量表与评分准则」，可改名称、分值、启用状态。两条要注意：\n' +
    ...
    },
    {
      ...
      answer() {
        return '「设置 → 量表与评分准则」的「智能量表」有两种用法：\n' +
    ...
    },
    {
      ...
      answer(ctx) {
        const t = ctx.docType;
        const head = '类型决定评分方向——用编程实验的量表去评化学实验，「代码实现」那几分永远拿不到，那不是分低，是评错了。\n' +
          '· 「设置 → 类型设置」内置 7 类（编程 / 物理电类 / 化学 / 机器学习 / 课程论文 / 社会调研 / 通用），都能改名、改特征词、增删评分方向\n' +
    ...
    },
    {
      ...
      answer(ctx) {
        const head = '在「设置 → 模型设置」填 Base URL / API Key / 模型名，兼容 OpenAI 风格的 /chat/completions' +
    ...
    {
      id: 'batch',
      keys: ['批量', '统计', '分布', '信度', '一致性', '交叉验证', '融合', '双模型', '平均分'],
      answer() {
        return '两个页面分管这些：\n' +
          '· 「批量与查重」：成绩总表、分数分布、两两相似度矩阵\n' +
          '· 「设置 → 分析与校验」：量表自动诱导、评分信度自检（含重复评阅稳定性）、双模型交叉验证与分数融合\n' +
          '· 「设置 → 类型设置」：内置 7 类文档类型与各自评分方向，可增删改，与工作台双向同步\n\n' +
          '两个模型的分歧会直接摆出来让你裁决，不会偷偷取个平均值糊弄过去。';
      },
    },
    {
      id: 'theme',
      ...
      answer() {
        return '「设置 → 个性化设置」可以切界面主题和评语人格。两条硬边界：\n' +
          '· 主题只改观感，不动分数\n' +
          '· 人格只改措辞，也不动分数\n\n' +
    ...
    },
    {
      ...
      answer() {
        return '「设置 → 分析与校验」的量表自动诱导：挑 2~3 份最好的当优秀组、2~3 份最差的当对照组，' +
          '引擎反推「哪些要点能把两组区分开」，按区分度 × √支持度排序生成量表。\n\n' +
          '不想手挑范文的话，用「设置 → 量表与评分准则」的「按已上传作业适配」——它自己排高低组，' +
          '还能处理混批（一批里夹杂不同题目的作业）。';
      },
    },
  ];
  ...
  function fallbackAnswer(ctx) {
    ...
    return base + (ctx && ctx.llmReady
      ...
      : '\n想要更自由的问答，去「设置 → 模型设置」配一个 API Key——配好之后，匹配不到的问题会转给大模型，并带上当前报告的评分上下文。');
  }
